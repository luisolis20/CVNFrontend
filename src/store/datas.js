...
export default {
  data() {
    return {
      ...
      url: "http://192.168.1.110/cvn/api/cvn/v1/users",
    };
  },
  watch: {
    claven(newVal) {
  ...
  methods: {
    ...
    async enviarComentario() {
      ...
      await axios.post('http://192.168.1.110/cvn/api/cvn/v1/enviar-comentario', comentarioData)
        .then(response => {
    ...
    async enviarCodigo() {
      try {
        ...
        if (this.email.trim() === '') {
          ...
          const response = await axios.post("http://192.168.1.110/cvn/api/cvn/v1/enviar-correo",
          {
            email: this.email.trim(),
            }
          );
